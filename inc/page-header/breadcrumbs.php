 ...
 */
function kraftiart_breadcrumbs() {

	$home_breadcrumbs = 0;
	$home             = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-home"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	$mycurrent        = 1;
	global $post;
	...
	if ( is_front_page() ) {
		...
		if ( is_home() ) {
			echo '<span class="active">' . esc_html__( 'Blog', 'kraftiart' ) . '</span>';
		} elseif ( is_category() ) {
			$this_cat = get_category( get_query_var( 'cat' ), false );
			echo '<span class="active">' . esc_html__( 'Archive : ', 'kraftiart' ) . ' "' . single_cat_title( '', false ) . '" </span>';
		} elseif ( is_search() ) {
			echo '<span class="active">' . esc_html__( 'Results : ', 'kraftiart' ) . ' "' . get_search_query() . '"</span>';
		} elseif ( is_day() ) {
			...
			echo '<span class="active">' . esc_html__( 'Posts tagged', 'kraftiart' ) . ' "' . single_tag_title( '', false ) . '"</span>';
		} elseif ( is_author() ) {
			global $author;
			$userdata = get_userdata( $author );
			echo '<span class="active">' . esc_html__( 'Articles By : ', 'kraftiart' ) . ' ' . $userdata->display_name . '</span>';
		} elseif ( is_404() ) {
			echo '<span class="active">' . esc_html__( '404', 'kraftiart' ) . '</span>';
		}
		if ( get_query_var( 'paged' ) ) {
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
				echo ' (';
			}
			echo esc_html__( 'Page', 'kraftiart' ) . ' ' . get_query_var( 'paged' );
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
				echo ')';
			}
		}
