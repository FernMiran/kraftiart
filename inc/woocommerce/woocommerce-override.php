...
if ( ! function_exists( 'woocommerce_widget_shopping_cart_button_view_cart' ) ) {
	...
	function woocommerce_widget_shopping_cart_button_view_cart() {
		echo '<a href="' . esc_url( wc_get_cart_url() ) . '" class="button wc-forward">' . esc_html__( 'View cart', 'kraftiart' ) . '</a>';
	}
}
...
?>
                <?php
                    ...
                    if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
                        ...
                        if( $kraftiart_options['products-wishlist-detail'] == 1 && is_product() ){ ?>
                            <div class="wishlist btn-hv" title="<?php esc_attr_e( 'Wishlist', 'kraftiart'); ?>"><?php echo do_shortcode( '[yith_wcwl_add_to_wishlist]' ); ?></div>
                        <?php } elseif( $kraftiart_options['products-wishlist-button'] == 1 ){
                            if( $kraftiart_options['products-wishlist'] == 1 ){ ?>
                                <div class="wishlist btn-hv" title="<?php esc_attr_e( 'Wishlist', 'kraftiart'); ?>"><?php echo do_shortcode( '[yith_wcwl_add_to_wishlist]' ); ?></div>
                            <?php }
                        }
                    } else { ?>
                ...
                <?php if( class_exists( 'YITH_Woocompare_Frontend' ) ):
                    if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
                        ...
                        if( $kraftiart_options['products-compare-detail'] == 1 && is_product() ){ ?>
                            <div class="compare btn-hv" title="<?php esc_attr_e( 'Compare', 'kraftiart'); ?>"><?php echo do_shortcode( '[yith_compare_button]' ); ?></div>
                        <?php } elseif( $kraftiart_options['products-compare-button'] == 1 ){
                            if( $kraftiart_options['products-compare'] == 1 ){ ?>
                                <div class="compare btn-hv" title="<?php esc_attr_e( 'Compare', 'kraftiart'); ?>"><?php echo do_shortcode( '[yith_compare_button]' ); ?></div>
                            <?php }
                        }
                    } else { ?>
                ...
                if ( class_exists( 'YITH_WCQV' )):
                    if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
                        $kraftiart_options = get_option( 'kraftiart_options' ); 
                        if( $kraftiart_options['products-quick-view'] == 1 ){ ?>
                            <div class="quickview btn-hv" title="<?php esc_attr_e( 'Quick View', 'kraftiart'); ?>"><?php echo do_shortcode( '[yith_quick_view]' ); ?></div>
                        <?php }
                    } else { ?>
                        <div class="quickview btn-hv" title="<?php esc_attr_e( 'Quick View', 'kraftiart'); ?>"><?php echo do_shortcode( '[yith_quick_view]' ); ?></div>
                <?php } endif; ?>
...
function wc_get_rate_html( $rating, $count = 0 ) {
	...
	if ( 0 < $rating ) {
		...
		$label = sprintf( esc_html__( 'Rated %s out of 5', 'kraftiart' ), $rating );
    ...
    // else{
    //     $label = sprintf( esc_html__( 'Rated %s out of 5', 'dazzing' ), $rating );
	// 	$html  = '<div class="wrap-rate"><div class="star-rating" role="img" aria-label="' . esc_attr( $label ) . '">' . wc_get_star_rating_html( $rating, $count ) . '</div></div>';
    // }
...
function wp_product_sold_count() {
	...
	if ( ! empty( $total_sold ) ){
        sprintf( esc_html__( '%s', 'kraftiart' ), $total_sold );
    }
}
...
add_action( 'woocommerce_after_single_product_summary', 'kraftiart_woocommerce_output_upsells', 15 );
function kraftiart_woocommerce_output_upsells() {
