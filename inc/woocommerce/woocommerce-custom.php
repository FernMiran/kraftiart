...
if ( ! function_exists( 'woocommerce_get_product_thumbnail' ) ) {
	...
	function woocommerce_get_product_thumbnail( $size = 'woocommerce_thumbnail', $deprecated1 = 0, $deprecated2 = 0 ) {
		...
		$image = wp_get_attachment_image_src( get_post_thumbnail_id( $product->get_id() ), 'medium', 'single-post-thumbnail' ); ?>
		...
		if( $gallery_image_ids = get_post_meta( $product->get_id(), '_product_image_gallery', true ) ) :
			...
			?><img src="<?php echo wp_get_attachment_image_url( reset($gallery_image_ids), 'medium', 'single-post-thumbnail'); ?>" data-id="<?php echo esc_html( $product->get_id() ); ?>" class="hover-img" alt="Hover">
		<?php endif; ?>
...
function kraftiart_woocommerce_setup() {
...
add_action( 'after_setup_theme', 'kraftiart_woocommerce_setup' );
 ...
 * @return void
 */
function kraftiart_woocommerce_scripts() {
	wp_enqueue_style( 'kraftiart-woocommerce-style', get_template_directory_uri() . '/woocommerce.css', array(), 'kraftiart_version' );
	...
	wp_add_inline_style( 'kraftiart-woocommerce-style', $inline_font );
}
add_action( 'wp_enqueue_scripts', 'kraftiart_woocommerce_scripts' );
 ...
 */
function kraftiart_woocommerce_active_body_class( $classes ) {
	$classes[] = 'woocommerce-active';

	return $classes;
}
add_filter( 'body_class', 'kraftiart_woocommerce_active_body_class' );
 ...
 */
function kraftiart_woocommerce_related_products_args( $args ) {
	$defaults = array(
		'posts_per_page' => -1,
		'columns'        => 5,
	);
...
add_filter( 'woocommerce_output_related_products_args', 'kraftiart_woocommerce_related_products_args' );

if ( ! function_exists( 'kraftiart_woocommerce_wrapper_before' ) ) {
	/**
	 * Before Main Content.
	 *
	 ...
	 * @return void
	 */
	function kraftiart_woocommerce_wrapper_before() {
		?>
...
add_action( 'woocommerce_before_main_content', 'kraftiart_woocommerce_wrapper_before' );

if ( ! function_exists( 'kraftiart_woocommerce_wrapper_after' ) ) {
	/**
	 * After Main Content.
	 *
	 ...
	 * @return void
	 */
	function kraftiart_woocommerce_wrapper_after() {
		?>
				</div>
			</div>
		</main><!-- #main -->
...
add_action( 'woocommerce_after_main_content', 'kraftiart_woocommerce_wrapper_after' );

/**
 * content wrap
 */
if ( ! function_exists( 'kraftiart_content_wrapper_before' ) ) {
	/**
	 * Before Content.
	 *
	 * @return void
	 */
	function kraftiart_content_wrapper_before() {
		echo '<div class="content-wrap">';
	}
}
add_action( 'woocommerce_before_content', 'kraftiart_content_wrapper_before', 10 );

if ( ! function_exists( 'kraftiart_content_wrapper_after' ) ) {
	/**
	 * After Content.
	 *
	 * @return void
	 */
	function kraftiart_content_wrapper_after() {
		echo '</div>'; /* #content-wrap */
	}
}
add_action( 'woocommerce_after_content', 'kraftiart_content_wrapper_after' );
 ...
 */
if ( ! function_exists( 'kraftiart_content_wrapper_before_list' ) ) {
	/**
	 * Before Content.
	 *
	 * @return void
	 */
	function kraftiart_content_wrapper_before_list() {
		echo '<div class="list-content-wrap col-8">';
	}
}
add_action( 'woocommerce_before_content_list', 'kraftiart_content_wrapper_before_list', 10 );

if ( ! function_exists( 'kraftiart_content_wrapper_after_list' ) ) {
	/**
	 * After Content.
	 *
	 * @return void
	 */
	function kraftiart_content_wrapper_after_list() {
			echo '</div>'; /* #content-wrap */
	}
}
add_action( 'woocommerce_after_content_list', 'kraftiart_content_wrapper_after_list' );
 ...
 */
if ( ! function_exists( 'kraftiart_content_wrapper_before_short' ) ) {
	/**
	 * Before Content.
	 *
	 * @return void
	 */
	function kraftiart_content_wrapper_before_short() {
		echo '<div class="list-content-wrap col-xl-10 col-md-9 col-8">';
	}
}
add_action( 'woocommerce_before_content_short', 'kraftiart_content_wrapper_before_short', 10 );
...
if ( ! function_exists( 'kraftiart_woocommerce_cart_link' ) ) {
	/**
	 * Cart Link.
	 *
	 ...
	 * @return void
	 */
	function kraftiart_woocommerce_cart_link() {
		?>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'kraftiart' ); ?>">
			<?php
			$item_count_text = sprintf(
				/* translators: number of items in the mini cart. */
				_n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'kraftiart' ),
...
if ( ! function_exists( 'kraftiart_woocommerce_header_cart' ) ) {
	/**
	 * Display Header Cart.
	 *
	 * @return void
	 */
	function kraftiart_woocommerce_header_cart() {
		if ( is_cart() ) {
		...
		<ul id="site-header-cart" class="site-header-cart">
			<li class="<?php echo esc_attr( $class ); ?>">
				<?php kraftiart_woocommerce_cart_link(); ?>
			</li>
			<li>
				<?php
				$instance = array(
...
function custom_mini_cart() {
	...
	echo '<svg xmlns="http://www.w3.org/2000/svg" height="20" width="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-shopping-cart"><circle cx="9" cy="21" r="1"></circle><circle cx="20" cy="21" r="1"></circle><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path></svg>';
	...
	echo '<div class="dropdown-menu-mini-cart">';
		echo '<div class="cart-slider">';
			echo '<div class="shopping_cart_top"><div class="cart-title">'.esc_html( 'Shopping Cart','kraftiart' ).'</div><div class="cart-close"><i class="fas fa-times"></i></div></div>
...
function sales_timer_countdown_product() {  
	...
	?>
    	    ...
    	    <!-- this is where the countdown is displayed -->
 	       	<div data-id="<?php echo esc_html( $product->get_id() ); ?>" class="timer">
   ...
   if ( $product->is_type( 'simple' ) ) {
      ...
      foreach ( $product->get_children() as $child_id ) {
   	...
   	if( class_exists( 'ReduxFramework' ) ) {
		...
		if( $kraftiart_options['products-sale-text'] == 0 ){
			if ( $max_percentage > 0 ) echo "<div class='kraftiart-sale'><span class='onsale'>-" . round($max_percentage) . "%</span></div>"; // If you would like to show -40% off then add text after % sign
		}else{
			if ( $max_percentage > 0 ) echo "<div class='kraftiart-sale'><span class='label'>". esc_html__( 'Sale', 'kraftiart' ) ."</span></div>"; // If you would like to show -40% off then add text after % sign
		}
	}
...
function ps_selectbox() {
	...
	if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
		$kraftiart_options = get_option( 'kraftiart_options' );
		$page_total = $kraftiart_options['num-product-page'];
	}else{
		$page_total = 12;
	}
...
function ps_pre_get_products_query( $query ) {
	...
	if( empty( $per_page_get ) ){
		if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
			$kraftiart_options = get_option( 'kraftiart_options' );
			if( ! empty( $kraftiart_options['num-product-page'] ) ){
				$per_pages = $kraftiart_options['num-product-page'];
			}
		}
	}else{
		...
		<div id="product-search-0" class="product-search floating">
			<div class="product-search-form">
				<form id="product-search-form-0" class="product-search-form show-submit-button" action="" method="get">
					<input id="product-search-field-0" name="s" type="text" class="product-search-field" placeholder="<?php esc_attr__( 'Search products …', 'kraftiart' ); ?>" autocomplete="off">
					...
					<button type="submit"><?php esc_html_e( 'Search', 'kraftiart' ); ?></button>
				</form>
			</div>
...
function woocommerce_offsidebar_filter(){
	if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
		...
		if( $shop_layout == "right" ){
			...
			$filter_class = $kraftiart_options['shop-layout'];
		}
	}else{
		$filter_class = "right";
	}
	if ( is_active_sidebar( 'shop' ) ) {
		echo '<div class="toggle-filter"><a class="filter '. esc_html( $filter_class ) .'">'. esc_html__( 'Filter', 'kraftiart' ) .'<i class="fas fa-align-right"></i></a></div>';
	}
}
...
function woocommerce_grid_list_view(){
	...
	if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
		$kraftiart_options = get_option( 'kraftiart_options' );
		$view_mode = !empty( $kraftiart_options['product-view-mode'] ) ? sanitize_text_field( $kraftiart_options['product-view-mode'] ) : '' ;
		$columns = $kraftiart_options['num-product-row'];
		...
		if( $view_mode == 1 || $view_mode == 2 ){
			echo '<a href="'. $shop_page_url .'?view=grid-4" id="grid" class="view grid '. $grid .'" title="'. esc_attr__( 'Grid View', 'kraftiart') .'">
			...
			echo '<a href="'. $shop_page_url .'?view=grid-3" id="grid-3" class="view grid-3 '. $grid3 .'" title="'. esc_attr__( '3 Column', 'kraftiart') .'">
			...
			echo '<a href="'. $shop_page_url .'?view=grid-2" id="grid-2" class="view grid-2 '. $grid2 .'" title="'. esc_attr__( '2 Column', 'kraftiart') .'">
			...
			echo '<a href="'. $shop_page_url .'?view=list-view" id="list" class="view list '. $list .'" title="'. esc_attr__( 'List View', 'kraftiart') .'">
			...
			echo '<a href="'. $shop_page_url .'?view=short-view" id="short" class="view short '. $short .'" title="'. esc_attr__( 'Short View', 'kraftiart') .'">
...
function new_loop_shop_per_page( $cols ) {
	if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
		$kraftiart_options = get_option( 'kraftiart_options' );
		$cols = $kraftiart_options['num-product-page'];
	}else{
		$cols = 12;
	}
	return $cols;
...
if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
	$kraftiart_options = get_option( 'kraftiart_options' );
	if( $kraftiart_options['products-catalog-mode'] == 1 ){
...
function woocommerce_add_custom_general_fields() {
	...
	woocommerce_wp_text_input(
		array(
			...
			'label' => esc_html__('Video Tutorial', 'kraftiart'),
			'placeholder' => 'Tutorial url',
			'description' => esc_html__('Has Video Tutorial?', 'kraftiart'),
...
function get_product_brand_name(){
	...
	if( !empty( $terms ) ){
		echo '<div class="single-categories">';
			echo '<span>'.esc_html__( 'Brands : ', 'kraftiart' ).'</span>';
			echo '<div class="kraftiart-brands">';
...
function get_product_estimated_delivery(){
	...
	if( !empty( $delivery ) ){
		echo '<div class="estimated-delivery">';
			echo '<span>'.esc_html__( 'Estimated Delivery : ', 'kraftiart' ).'</span>';
...
function get_product_free_shipping(){
		...
		if( !empty( $shipping ) ){
			echo '<div class="free-shipping">';
				echo '<span>'.esc_html__( 'Free Shipping : ', 'kraftiart' ).'</span>';
...
?>
    <div class="form-field term-group">
        ...
        <label for="image_id"><?php esc_html__('Image', 'kraftiart'); ?></label>
        <input type="hidden" id="image_id" name="image_id" class="custom_media_url" value="">

        <div id="image_wrapper"></div>

        <p>
            <input type="button" class="button button-secondary taxonomy_media_button" id="taxonomy_media_button" name="taxonomy_media_button" value="<?php esc_attr__( 'Add Image', 'kraftiart' ); ?>">
            <input type="button" class="button button-secondary taxonomy_media_remove" id="taxonomy_media_remove" name="taxonomy_media_remove" value="<?php esc_attr__( 'Remove Image', 'kraftiart' ); ?>">
        </p>
...
function kraftiart_social_share_button(){
	if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
		...
		<div class="xs_social_share_widget xs_share_url main_content  wslu-style-3 wslu-share-box-shaped wslu-fill-brand-hover-colored wslu-share-m-5 wslu-none wslu-share-horizontal wslu-theme-font-no wslu-main_content">
			<ul>
				<?php
				if( $kraftiart_options['opt_fb_shar'] == 1 ){ ?>
					<li>
						<a target="_blank" title="Share On Facebook" href="https://www.facebook.com/sharer.php?u=<?php the_permalink(); ?>"><div class="xs-social-icon"><i class="fa-brands fa-facebook-f"></i></div></a>
					</li>
				<?php }
				if( $kraftiart_options['opt_twitter_shar'] == 1 ){ ?>
					<li>
						<a target="_blank" title=" Share On Twitter" href="https://twitter.com/share?url=<?php the_permalink(); ?>"><div class="xs-social-icon"><i class="fa-brands fa-twitter"></i></div></a>
					</li>
				<?php }
				if( $kraftiart_options['opt_linkedin_shar'] == 1 ){ ?>
					<li>
						<a target="_blank" title=" Share On Linkedin" href="https://linkedin.com/shareArticle?mini=true&amp;url=<?php the_permalink(); ?>"><div class="xs-social-icon"><i class="fa-brands fa-linkedin-in"></i></div></a>
					</li>
				<?php }
				if( $kraftiart_options['opt_pinterest_shar'] == 1 ){ ?>
					<li>
						<a target="_blank" title=" Share On Pinterest"  href="https://pinterest.com/pin/create/button/?url=<?php the_permalink(); ?>"><div class="xs-social-icon"><i class="fa-brands fa-pinterest-p"></i></div></a>
					</li>
				<?php }
				if( $kraftiart_options['opt_tumblr_shar'] == 1 ){ ?>
					<li>
						<a target="_blank" title=" Share On Tumblr"  href="https://tumblr.com/share/link?url=<?php the_permalink(); ?>"><div class="xs-social-icon"><i class="fa-brands fa-tumblr"></i></div></a>
					</li>
				<?php }
				if( $kraftiart_options['opt_email_shar'] == 1 ){ ?>
					<li>
						<a target="_blank" title=" Share On Envelope" href="https://mailto:?subject=<?php the_permalink(); ?>"><div class="xs-social-icon"><i class="fa-regular fa-envelope"></i></div></a>
					</li>
				<?php }
				if( $kraftiart_options['opt_whats_shar'] == 1 ){ ?>
					<li>
						<a target="_blank" title=" Share On Whatsapp" href="whatsapp://send?text=<?php the_permalink(); ?>"><div class="xs-social-icon"><i class="fa-brands fa-whatsapp"></i></div></a>
					</li>
				<?php } ?>
			</ul>
		</div>
...
add_action( 'kraftiart_social_share', 'kraftiart_social_share_button'  );
...
function custom_stock_progressbar(){
	...
	if( $available_stock > 0 ){
		...
		echo '<div class="stock-progress">';
			echo '<span class="stock-progress-wrap">'. esc_html( 'Only '. $available_stock .' item(s) left in stock!', 'kraftiart' ).'</span>';
...
function single_products_summary(){
	...
	if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
		...
		if( $thumbnail_slider == 1 ){
			...
			remove_action( 'woocommerce_after_single_product_summary', 'kraftiart_woocommerce_output_upsells', 15 );
...
function custom_checkbox_field_product_options_sku(){
    ...
    woocommerce_wp_checkbox( array(
        ...
        'label'         => esc_html__( 'Deal Of The Product', 'kraftiart' ),
        'description'   => esc_html__( 'This product will be shown on the product deals of the day.', 'kraftiart' ),
    ) );
}
			...
			foreach( $taxonomies as $prod_cat ) :
				echo '<div class="category-item swiper-slide">';
					...
					if( ! empty( $shop_catalog_img ) ){
						...
						$cat_image = '<img src="'. KRAFTIARTE_TH_URL .'/assets/images/woocommerce-placeholder.png" alt="placeholder">';
					}
		...
		<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
			<thead>
				<tr>
					...
					<th class="product-name"><?php esc_html_e( 'Product', 'kraftiart' ); ?></th>
					<th class="product-price"><?php esc_html_e( 'Price', 'kraftiart' ); ?></th>
					<th class="product-quantity"><?php esc_html_e( 'Quantity', 'kraftiart' ); ?></th>
					<th class="product-subtotal"><?php esc_html_e( 'Subtotal', 'kraftiart' ); ?></th>
				</tr>
			</thead>
			<tbody>
				...
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
								...
								<?php
									echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
										...
										sprintf(
											...
											esc_html__( 'Remove this item', 'kraftiart' ),
											esc_attr( $product_id ),
							...
							<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'kraftiart' ); ?>">
								<?php
								if ( ! $product_permalink ) {
								...
								if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
									echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'kraftiart' ) . '</p>', $product_id ) );
								}
								?>
							</td>
							<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'kraftiart' ); ?>">
								<?php echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key ); // PHPCS: XSS ok.?>
							</td>
							<td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'kraftiart' ); ?>">
							...
							<td class="product-subtotal" data-title="<?php esc_attr_e( 'Subtotal', 'kraftiart' ); ?>">
								<?php
				...
				<tr>
					<td colspan="6" class="actions">
						<?php if ( wc_coupons_enabled() ) { ?>
							<div class="coupon">
							<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'kraftiart' ); ?>" /> <button type="submit" class="button btn btn-primary" name="apply_coupon" value="<?php esc_attr_e( 'Apply coupon', 'kraftiart' ); ?>"><?php esc_html_e( 'Apply coupon', 'kraftiart' ); ?></button>
								<?php do_action( 'woocommerce_cart_coupon' ); ?>
							</div>
						<?php } ?>
						<button type="submit" class="button btn btn-primary" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'kraftiart' ); ?>"><?php esc_html_e( 'Update cart', 'kraftiart' ); ?></button>
