 ...
 */
add_action( 'woocommerce_shop_loop_item_attribute', 'kraftiart_product_attributes', 10 );
function kraftiart_product_attributes(){
 ...
 */
function kraftiart_get_ajax_image(){
	...
	foreach ( $variations as $variation ) {
		...
		if( $variation['attributes']['attribute_pa_color'] == $dataid ){
			...
			if( $variation['attributes']['attribute_pa_size'] == $already_selected ){
				...
				$add_to_cart = '<a href="?add-to-cart='. $id .'" data-quantity="1" class="button product_type_simple add_to_cart_button ajax_add_to_cart product-button" data-product_id="'. $variation['variation_id'] .'"  rel="nofollow">'. esc_html( 'Add to cart', 'kraftiart' ) .'</a>';
...
add_action('wp_ajax_get_ajax_image', 'kraftiart_get_ajax_image');
add_action('wp_ajax_nopriv_get_ajax_image', 'kraftiart_get_ajax_image');
