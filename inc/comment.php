...
/**
 * kraftiart Comment Function
 */
function kraftiart_comments( $comment, $args, $depth ) {
	...
	switch ( $comment->comment_type ) :
		...
		case 'trackback':
			?>
		<div class="pingback-entry"><span class="pingback-heading"><?php esc_html_e( 'Pingback:', 'kraftiart' ); ?></span> <?php comment_author_link(); ?></div>
			<?php
			break;
		default:
			if ( 'div' != $args['style'] ) {
					...
					<div class="comment-wrap d-inline-block overflow-hidden flex-grow-1">
						<div class="comment-author vcard">
							<div class="comment-meta-wrap">
								<?php
								// Display author name.
								printf( __( '<cite class="fn">%s</cite>', 'kraftiart' ), get_comment_author_link() );
								?>
							</div>
						</div><!-- .comment-author -->
						<div class="comment-details">
							...
							if ( $comment->comment_approved == '0' ) {
								?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'kraftiart' ); ?></em><br/>
																				<?php
							}
							?>
						</div><!-- .comment-details -->
						<div class="comment-meta commentmetadata float-start w-100 d-flex align-items-center">
							...
							<?php
							edit_comment_link( __( '(Edit)', 'kraftiart' ), '  ', '' );
							?>
 ...
 * @param string $fields .
 */
function kraftiart_move_comment_form_below( $fields ) {
...
add_filter( 'comment_form_fields', 'kraftiart_move_comment_form_below' );
