 ...
 */
function kraftiart_myscript() {
	$kraftiart_options= get_option( 'kraftiart_options' );
	if ( ! empty( $kraftiart_options['js-code'] ) ) {
		?>
	<script>
		<?php echo wp_specialchars_decode( $kraftiart_options['js-code'] ); ?>
	</script>
		<?php
	}
}
add_action( 'wp_footer', 'kraftiart_myscript', 100 );

/**
 * theme option js in header
 */
function kraftiart_myscript_jquery() {
	$kraftiart_options= get_option( 'kraftiart_options' );
	if ( ! empty( $kraftiart_options['js-header-code'] ) ) {
		?>
	<script>
		<?php echo wp_specialchars_decode( $kraftiart_options['js-header-code'] ); ?>
	</script>
		<?php
	}
}
add_action( 'wp_head', 'kraftiart_myscript_jquery', 100 );
