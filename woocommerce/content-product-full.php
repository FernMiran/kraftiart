...
echo '<div class="col-12 full-sidebar shop-nosidebar">';
	if ( woocommerce_product_loop() ) {
		...
		if( class_exists( 'ReduxFramework' ) ) {
			...
			if( $kraftiart_options['shop-page-display'] == "categories" || $kraftiart_options['shop-page-display'] == "both" ){
				do_action( 'woocommerce_shop_loop_categories' );
			}
			if( $kraftiart_options['category-page-display'] == "subcategories" || $kraftiart_options['category-page-display'] == "both" ){
		...
		if ( wc_get_loop_prop( 'total' ) ) {
			while ( have_posts() ) {
				...
				if( class_exists( 'ReduxFramework' ) && function_exists( 'loadmore_postSC' ) ) {
					if( is_shop() ){
						if( $kraftiart_options['shop-page-display'] == "products" || $kraftiart_options['shop-page-display'] == "both" ){
							$kraftiart_options = get_option( 'kraftiart_options' );
							...
							}elseif( $kraftiart_options['product-layout'] == "product-layout-default" ){
								wc_get_template_part( 'content', 'product' );
							}
						}
					}
					if( ! is_shop() ){
						if( $kraftiart_options['category-page-display'] == "products" || $kraftiart_options['category-page-display'] == "both" ){
